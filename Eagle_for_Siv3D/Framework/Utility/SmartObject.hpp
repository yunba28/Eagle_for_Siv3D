...
{
	template<class T>
	using UniqueObject = std::unique_ptr<T>;
	...
	public:
		...
		WeakObject& operator=(const WeakObject&) = default;

		template<class U>
		WeakObject& operator=(const WeakObject<U>& _other);

		WeakObject& operator=(WeakObject&&) = default;

		template<class U>
		WeakObject& operator=(WeakObject<U>&& _other);
...
namespace eagle
{
	...
	template<class T>
	template<class U>
	inline WeakObject<T>& WeakObject<T>::operator=(const WeakObject<U>& _other)
	{
		mWeak = _other.mWeak;
		return *this;
	}
	...
	template<class T>
	template<class U>
	inline WeakObject<T>& WeakObject<T>::operator=(WeakObject<U>&& _other)
	{
		mWeak = forward<std::weak_ptr<U>>(_other.mWeak);
		return *this;
	}
